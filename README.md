...
vault (.md files)
      |
      v
  loader.py        parses frontmatter, produces Document objects
      |
      v
  chunkers.py       splits each document (3 interchangeable strategies)
      |
      v
  embedder.py       calls Foundry Local, caches vectors on disk
      |
      v
  store.py          persists to .npy + .jsonl, brute-force cosine search
      |
      v
  query.py          retrieves top-k, then reranker.py gates on confidence
```
...
**Foundry Local for embeddings, via its HTTP endpoint rather than the Python
SDK.** The embedding provider is Microsoft Foundry Local
(`qwen3-embedding-0.6b`, CUDA GPU variant, 1024-dim). Two issues surfaced
along the way and are documented here since they cost real debugging time:
...
**Empirical context limit, not the advertised one.** The catalog reports
`context_length: 32768` tokens, but in practice the CUDA embedding endpoint
returns HTTP 500 past roughly 1,500-2,000 words. This was found by embedding
the full 91-file corpus and bisecting the failure on the largest file
(`ml-pipeline.md`, 2,813 words). `embedder.py` truncates to 1,200 words with
logging (7 of 91 whole-document chunks were affected) rather than failing
silently or trusting the documented limit.

**No framework — LangChain was deliberately not used.** The embedding API is
a single HTTP call, and storage is plain `.npy` + `.jsonl`: for this corpus
size, the embedding matrix is a few megabytes and brute-force cosine
similarity (`matrix @ vector`) runs in sub-millisecond time. A vector
database would solve a scaling problem this project does not have; adding
one would add a dependency, a schema, and an approximation error for no
measurable benefit at this scale.

**Retrieval-only evaluation.** `query.py` returns ranked sources without
generating a natural-language answer. This keeps the evaluated variable
(chunking strategy) isolated from a constant, unimplemented generation
step. A chat model (`qwen2.5-0.5b`) is available in Foundry Local and could
be added later without touching retrieval or evaluation code.
...
feedback loop is not hypothetical" below).
Minimising a stated cost function on that pool — answering an out-of-scope
question is weighted twice as costly as refusing an in-scope one — and
validating with 5-fold cross-validation reports 3/33 false accepts and 5/47
false refuses *on questions the threshold was not fitted on*
...
help. A min-max-normalized weighted sum of bi-encoder and TF-IDF scores
underperforms the bi-encoder alone at every weight tested (HitRate@1 drops
from 0.809 to between 0.702 and 0.766 depending on how much weight TF-IDF
gets). The reason ties back to the corpus: hybrid search is usually
